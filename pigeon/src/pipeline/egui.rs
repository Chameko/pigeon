...
impl EguiPipe {
    ...
    pub fn update_buffers(&mut self, full_meshes: Vec<egui::Mesh>, paint: &mut Painter, logical_size: Size2D<u32, ScreenSpace>) {
        ...
        for (i, mesh) in meshes.iter().enumerate() {
            ...
            if i < self.index_buffer.len() {
                ...
                if mesh.indices.len() > self.index_buffer[i].elements as usize {
                    self.index_buffer[i] = paint.index_buffer(&mesh.indices, Some("Egui index buffer"));
                } else {
                    paint.update_index_buffer(mesh.indices.as_slice(), &mut self.index_buffer[i])
                }
            } else {
                self.index_buffer.push(paint.index_buffer(mesh.indices.as_slice(), Some("Egui index buffer")));
            }
            ...
            if i < self.vertex_buffer.len() {
                if bytemuck::cast_slice::<_, u8>(&mesh.vertices).len() > self.vertex_buffer[i].size as usize {
                    ...
                    paint.update_vertex_buffer(mesh.vertices.as_slice(), &mut self.vertex_buffer[i]);
                }
            } else {
...
impl<'a> Plumber<'a> for EguiPipe {
    ...
    fn description() -> PipelineDescription<'a> {
        PipelineDescription {
            ...
            shader: ShaderFile::Wgsl(include_str!("../shader/egui.wgsl"))
        }
    }

    fn prepare(&'a mut self, context: Self::PrepareContext) -> Vec<(&'a mut UniformBuffer, Vec<Self::Uniforms>)> {
        let (width, height) = (context.width as f32, context.height as f32);
        let uniform = TransformUniform {
            screen_size: [width, height],
        };
        vec![(&mut self.uniform_buffer, vec![uniform])]
    }

    fn setup(pipe: Pipeline, device: &Device) -> Self {
        let vertex_buffer = vec![device.create_vertex_buffer::<Vertex>(&[], Some("Egui vertex buffer"))];
        let index_buffer = vec![device.create_index_buffer(&[], Some("Egui index buffer"))];
        let uniform_buffer = device.create_uniform_buffer::<TransformUniform>(&[], Some("Egui uniform buffer"));
